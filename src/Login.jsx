...
function Login({ setIsLoggedIn }) {
  ...
  const login = (e) => {
    ...
    if (storedUser?.name === form.name && storedUser?.password === form.password) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-green-100 px-4">
      <form onSubmit={login} className="bg-white w-full max-w-sm p-6 rounded-lg shadow-lg space-y-4">
        ...
        <input name="name" type="text" required placeholder="name" className="w-full px-3 py-2 border rounded" onChange={typing}/>
